...
* [`src/immersa/styles.cljs`](src/immersa/styles.cljs): CSS compilation source file (ClojureScript,
[Garden](https://github.com/noprompt/garden))
* [`src/immersa/`](src/immersa/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/immersa/core.cljs): contains the SPA entry point, `init`
* [`test/immersa/`](test/immersa/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
5. Clone this repo and open a terminal in the `immersa` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/immersa/config.cljs) defaults to `true` in
...
```clj
(ns immersa.example
  (:require [immersa.config :as config])
